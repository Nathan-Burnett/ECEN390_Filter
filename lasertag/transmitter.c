...
// This was used in an initial test of our code before using the interrupt
// method
#define TRANSMITTER_TEST_TICK_PERIOD_IN_MS 1
// This is to make sure that we are doing the right interval start and stops in
// non-continuous mode run
...
void transmitter_setFrequencyNumber(uint16_t frequencyNumber) {
  frequency_number =
      frequencyNumber; // reset the global variable with the passed in variable
}
...
void debugTransmitterStatePrint() {
  ...
  if (transmitter_previousState != transmitter_currentState || firstPass) {
    ...
    transmitter_previousState =
        transmitter_currentState; // keep track of the last state that you were
                                  // in.
    // printf("msCounter:%d\n\r", msCounter);
    switch (transmitter_currentState) { // This prints messages based upon the
...
void transmitter_tick() {
  ...
  case init_st:
    ...
    waveform_transmit_counter =
        TRANSMITTER_INITIALIZER; // just in case it somehow didnt work,
                                 // initialize the counter
    transmit_low_high_counter =
        TRANSMITTER_INITIALIZER; // same initialization to be sure
    break;
  ...
  case wait_to_transmit_st:
    ...
    if (begin_transmitting || continuous_mode) {
      transmitter_currentState =
          transmit_high_st; // start by going to the transmit high
      acting_frequency = frequency_number; // update the acting frequency with
                                           // the set frequency number
      begin_transmitting = false; // turn off our begin transmission flag
      is_transmitting = true; // turn on the fact that we're transmitting flag
      mio_writePin(TRANSMITTER_OUTPUT_PIN,
                   TRANSMITTER_HIGH); // set the output pin as high
    }
    break;

  case transmit_high_st:
    if (waveform_transmit_counter >
        TRANSMITTER_WAVEFORM_LENGTH) { // we have hit the end of the 200 ms
                                       // transmission go back to wait for
                                       // transmit state
      transmitter_currentState = wait_to_transmit_st; // go to wait to transmit
      is_transmitting = false;                        // no longer transmitting
      waveform_transmit_counter =
          TRANSMITTER_INITIALIZER; // reset the length counter
      transmit_low_high_counter =
          TRANSMITTER_INITIALIZER; // reset the low high counter
      mio_writePin(TRANSMITTER_OUTPUT_PIN,
                   TRANSMITTER_LOW); // make sure our output pin is set to low
    } // also check to see if we've transmitted high enough
    else if (transmit_low_high_counter >=
             (filter_frequencyTickTable[acting_frequency] *
              TRANSMITTER_DUTY_CYCLE)) { // if it has transmitted half the duty
                                         // cycle, switch to transmit low
      transmitter_currentState = transmit_low_st; // transition to transmit low
      mio_writePin(TRANSMITTER_OUTPUT_PIN,
                   TRANSMITTER_LOW); // set transmitter value to low
      transmit_low_high_counter =
          TRANSMITTER_INITIALIZER; // reset our high low transmit counter
    }
    break;

  case transmit_low_st:
    if (waveform_transmit_counter >
        TRANSMITTER_WAVEFORM_LENGTH) { // we have hit the end of the our 200 ms
                                       // transmission go back to wait for
                                       // transmit state
      ...
      waveform_transmit_counter =
          TRANSMITTER_INITIALIZER; // reset the length counter
      transmit_low_high_counter =
          TRANSMITTER_INITIALIZER; // reset the high and low counter
      mio_writePin(TRANSMITTER_OUTPUT_PIN,
                   TRANSMITTER_LOW); // reset the pin to low
    } else if (transmit_low_high_counter >=
               (filter_frequencyTickTable[acting_frequency] *
                TRANSMITTER_DUTY_CYCLE)) { // if we have transmitted half the
                                           // duty cycle, switch to transmit low
      transmitter_currentState =
          transmit_high_st; // transition back to transmit high
  ...
  case transmit_high_st:
    waveform_transmit_counter++; // increment the counters for waveform
                                 // transmission
  ...
  case transmit_low_st:
    waveform_transmit_counter++; // increment the counters for waveform
                                 // transmission and high low counter.
    transmit_low_high_counter++; // increment the counters for high low counter.
    break;
...
void transmitter_runTest() {
  ...
  while (!(buttons_read() &
    ...
    // we use this run on the non-continuous mode, not needed for continuous
    // mode transmitter_run(); use this on the non-continuous mode, not for
    // continuous mode
    transmitter_run();
...
// transmits one waveform and stops. To set continuous mode, you must invoke
...
// NOTE: while running continuously, the transmitter will change frequencies at
// the end of each 200 ms waveform.
void transmitter_setContinuousMode(bool continuousModeFlag) {
  continuous_mode = continuousModeFlag;
}

// This is provided for testing as explained in the transmitter section of the
// web-page. When enabled, debug prints are enabled to help to demonstrate the
// behavior of the transmitter.
...
// This is provided for testing as explained in the transmitter section of the
// web-page. When disabled, debug prints that were previously enabled no longer
// appear.
